...
class polarfireGpio:
    ...
    def pull_reset_low(self, pin):
        self._gpio.set_direction(pin, self._gpio.OUT)
        self._gpio.set_value(pin, self._gpio.LOW)

    def pull_reset_high(self, pin):
        self._gpio.set_direction(pin, self._gpio.OUT)
        self._gpio.set_value(pin, self._gpio.HIGH)
...
class adcam:
    ...
    def __init__(
        ...
        self._hololink = hololink_channel.hololink()
        self._i2c = self._hololink.get_i2c(hololink_i2c_controller_address)
        self._byteperpixel = 5
        self._width = 512*self._byteperpixel
        self._height = 512
        self._mode = 3  # mode
    ...
    def set_param(self, mode):
        logging.debug("Setting capture params as per mode")

        # Validate mode range: must be >0 and <=6
        if not (0 <= mode <= 6):
            raise ValueError(f"Invalid mode {mode}. Mode must be between 1 and 6.")

        if mode < 2:
            self._byteperpixel = 4
            self._width = 1024*self._byteperpixel
            self._height = 1024            
        else:
            self._byteperpixel = 5
            self._width = 512*self._byteperpixel
    ...
    def set_mode(self, mode):
        ...
        # Clear the mode field (bits 23:16)
        register &= 0xFF00FFFF

        # Insert new mode value
        register |= (mode & 0xFF) << 16
    ...
    def adcam_Only_reset(self, hololink, hololink_channel, channel_metadata):
        ...
        # time.sleep(10)  # Bootup ADSD3500
        """
        Toggle reset GPIO pins from 0 to 31:
        - Drive each pin LOW
        - Wait
        - Drive each pin HIGH
        - Final boot wait
        ...
        logging.debug("ADCAM - Reset sequence for GPIO")

        # ----------------------------------------------------------------------
        # Step 1: Drive all pins LOW
        # ----------------------------------------------------------------------
        self._pf_gpio.pull_reset_low(16)

        # Small delay after setting LOW
        time.sleep(1)
        logging.debug("ADCAM - now about to make GPIOs high")

        # ----------------------------------------------------------------------
        # Step 2: Drive all pins HIGH
        # ----------------------------------------------------------------------
        self._pf_gpio.pull_reset_high(16)

        # ----------------------------------------------------------------------
        # Step 3: Boot delay (ADSD3500 startup time)
        # ----------------------------------------------------------------------
        logging.info("Waiting 10 secs after reset (ADSD boot-up)")
        time.sleep(10)
    ...
    def start(self):
        ...
        if 0:  # mode[0] == 1:
            logging.debug("Contineous clock mode already enabled")
        else:
