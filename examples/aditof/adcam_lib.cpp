...
Adcam::Adcam(std::shared_ptr<hololink::DataChannel> hololink_channel,
    ...
    : hololink_(hololink_channel->hololink()),
      ...
      pf_gpio_(hololink_, channel_metadata, reset_pin /*pin*/),
...
void Adcam::i2c_write_read(std::shared_ptr<hololink::sensors::Adcam> adcam_inst,
  ...
  if (!adcam_inst || !adcam_inst->i2c_) {
    throw std::runtime_error("adcam_inst or adcam_inst->i2c_ is null");
  }
...
std::vector<uint8_t> Adcam::set_register16_response(
