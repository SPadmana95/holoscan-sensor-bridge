...
bool Adsd3500::adsd3500_flash(const std::vector<uint8_t>& file_data,
    ...
    std::cout << "[INFO] Master Chip ID is: 0x"
              << std::hex << std::uppercase << std::setw(4) << std::setfill('0')
              << master_chip_id << std::dec << std::endl;
    ...
    if (slave_found) {
        std::cout << "[INFO] Slave Chip ID is: 0x"
                  << std::hex << std::uppercase << std::setw(4) << std::setfill('0')
                  << slave_chip_id << std::dec << std::endl;
    } else {
...
bool Adsd3500::updateAdsd3500MasterFirmware(uint8_t *fw_data, uint32_t fw_len, bool force, uint32_t expected_crc) {
    ...
    uint8_t current_ver[44] = {0};
    {
        ...
        if (ver.size() != 44) {
            std::cerr << "[MASTER] Failed to read current firmware version (got "
            ...
            std::cout << "[MASTER] Current firmware version  : "
                      ...
                      << (int)current_ver[2] << "." << (int)current_ver[3] << std::endl;
        }
    }
    ...
    if (fw_len >= 4) {
        ...
        for (int i = 0; i < 4; i++) {
            if (fw_data[i] < min_ver[i]) { below_minimum = true;  break; }
            if (fw_data[i] > min_ver[i]) { below_minimum = false; break; }
        }
        if (below_minimum) {
    ...
    std::cout << "[MASTER] After upgrading new firmware " << std::endl;
    {
        auto ver = adcam_->get_fw_version_burst_mode(GET_MASTER_FIRMWARE_COMMAND);
        if (ver.size() != 44) {
            std::cerr << "[MASTER] Failed to read updated firmware version (got "
                      << ver.size() << " bytes, expected 44)" << std::endl;
        } else {
            std::cout << "[MASTER] Updated firmware version   : "
                      << (int)ver[0] << "." << (int)ver[1] << "."
...
bool Adsd3500::updateAdsd3500SlaveFirmware(uint8_t *fw_data, uint32_t fw_len, bool force, uint32_t expected_crc) {
    ...
    std::cout << "[SLAVE] After upgrading new firmware " << std::endl;
    {
        ...
        if (ver.size() != 44) {
            ...
            std::cout << "[SLAVE] Updated firmware version    : "
                      << (int)ver[0] << "." << (int)ver[1] << "."
                      << (int)ver[2] << "." << (int)ver[3] << std::endl;
        }
    }
...
bool Adsd3500::write_payload(uint8_t *payload, uint16_t payload_len) {
    if (payload_len == 0 || payload_len % 2 != 0) {
        std::cerr << "write_payload: payload_len must be a non-zero even number, got "
...
bool Adsd3500::read_burst_cmd(uint8_t *payload, uint16_t payload_len,
    ...
    if (payload_len == 0 || payload_len % 2 != 0) {
        std::cerr << "read_burst_cmd: payload_len must be a non-zero even number, got "
                  << payload_len << std::endl;
        return false;
    }
    uint16_t word_count = payload_len / 2;
