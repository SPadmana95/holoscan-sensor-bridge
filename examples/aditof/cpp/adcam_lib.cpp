...
encode_register_blob(const uint16_t *register_blob) {
...
static void log_reply_prefix(const char *label,
                             const std::vector<uint8_t> &reply) {
    if (reply.size() >= 2) {
        HOLOSCAN_LOG_INFO("{} bytes={} {:02X} {:02X}", label, reply.size(),
...
ADII2CExpander::ADII2CExpander(std::shared_ptr<hololink::Hololink> hololink_,
                               uint32_t i2c_bus, uint32_t expander_addr)
...
Adcam::Adcam(std::shared_ptr<hololink::DataChannel> hololink_channel,
             ...
             hololink::Metadata &channel_metadata, uint32_t adcam_mode,
             uint32_t reset_pin)
    : hololink_(hololink_channel->hololink()),
      ...
      pf_gpio_(hololink_, channel_metadata, reset_pin), adcam_mode_(adcam_mode),
...
std::vector<uint16_t>
Adcam::bytes_to_uint16_array_be(const std::vector<uint8_t> &data) {
    if (data.size() % 2 != 0) {
        throw std::runtime_error(
            "Data length must be even for 16-bit conversion.");
    }
...
void Adcam::i2c_write_read(const std::vector<uint8_t> &write_bytes,
    ...
    auto reply = i2c_->i2c_transaction(ADCAM_I2C_BUS_ADDRESS, write_bytes,
                                       read_byte_count, timeout);
...
bool Adcam::set_register16_no_response(uint16_t *register_blob) {
    try {
        ...
        HOLOSCAN_LOG_DEBUG("ADCAM REGISTER NORESPREQD total_size={}",
                           write_bytes.size());
...
std::vector<uint8_t> Adcam::set_register16_response(uint16_t *register_blob,
    ...
    try {
        ...
        HOLOSCAN_LOG_DEBUG("ADCAM REGISTER RESPREQD total_size={}",
                           write_bytes.size());
        i2c_write_read(write_bytes, resp_len, &reply);

        HOLOSCAN_LOG_DEBUG("ADCAM REGISTER REPLY bytes={}", reply.size());
    } catch (const std::exception &e) {
        holoscan::log_error("[ERROR] set_register16_response failed: {}",
                            e.what());
    }
...
void Adcam::set_mipi(std::shared_ptr<hololink::sensors::Adcam> adcam_inst) {
    ...
    uint16_t reg[] = {2, MIPI_OUTPUT_SPEED_CMD,
                      MIPI_SPEED_1GBPS}; // 1 Gbps configuration
...
void Adcam::read_nvm_config() {
    ...
    holoscan::log_warn("read_nvm_config: register blobs >64-bit need a "
...
void Adcam::configure_converter(
    ...
    converter->configure(start_byte, received_line_bytes, width_, height_,
                         csi::PixelFormat::RAW_8);
}

uint32_t Adcam::get_mode() { return adcam_mode_; }

uint16_t Adcam::get_imager_type() { return imager_type_; }

uint32_t Adcam::get_width() { return width_; }

uint32_t Adcam::get_height() { return height_; }

uint32_t Adcam::get_pixel_width() { return pixel_width_; }

uint32_t Adcam::get_pixel_height() { return pixel_height_; }
