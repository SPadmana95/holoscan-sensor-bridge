...
void Adcam::get_imager_type_and_ccb_version() {
  ...
  if (!mode_cfg) {
    HOLOSCAN_LOG_ERROR("get_imager_type_and_ccb_version: mode {} not found for "
...
std::vector<uint8_t> Adcam::get_fw_version(uint8_t cmd) {
  ...
  uint16_t reg3[] = {8, 0xAD00, 0x2C05, 0x0000, 0x0000, 0x3100, 0x0000, static_cast<uint16_t>(cmd << 8), 0x0000};
...
std::vector<uint8_t> Adcam::get_fw_version_burst_mode(uint8_t cmd) {
  ...
  auto resp = set_register16_response(reg3, 44);
  if (resp.size() < 44) {
    HOLOSCAN_LOG_ERROR("get_fw_version_burst_mode: incomplete response bytes={} (expected 44)",
                       resp.size());
  }
  return resp;
}
