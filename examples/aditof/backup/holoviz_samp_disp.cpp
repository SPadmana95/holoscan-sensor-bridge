...
public:
    ...
    void compose() override {
        ...
        adcam_inst->set_mipi(adcam_inst);
        adcam_inst->set_mode();
...
int main(int argc, char** argv)
{
    ...
    while (true) {
        ...
        if (c == 0) {
            ...
            if (opt->name == std::string("camera-mode")) {
            ...
            } else if (opt->name == std::string("expander-configuration")) {
                expander_configuration = std::stoul(argument);

            } else if (opt->name == std::string("pattern")) {
    ...
    try {
        ...
        //--------------------------------------------------------------------------
        auto adcam_inst =
            std::make_shared<hololink::sensors::Adcam>(hololink_channel, bus, channel_metadata);
